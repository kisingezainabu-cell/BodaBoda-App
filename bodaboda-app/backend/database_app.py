...
class BodaBodaDatabaseHandler(BaseHTTPRequestHandler):
    ...
    def create_tables(self):
        ...
        self.cursor.execute('''
            CREATE TABLE IF NOT EXISTS rides (
                id SERIAL PRIMARY KEY,
                ride_request_id INTEGER REFERENCES ride_requests(id) ON DELETE CASCADE,
                driver_id INTEGER REFERENCES users(id) ON DELETE CASCADE,
                start_time TIMESTAMP,
                end_time TIMESTAMP,
                actual_fare DECIMAL(10,2),
                distance_km DECIMAL(8,2),
                status VARCHAR(20) DEFAULT 'accepted' CHECK(status IN ('accepted', 'in_progress', 'completed', 'cancelled')),
                created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
                updated_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP
            )
        ''')
    ...
    def verify_jwt_token(self, token):
        """Verify JWT token"""
        try:
            return jwt.decode(token, os.environ.get('JWT_SECRET', 'bodaboda-secret-key'), algorithms=['HS256'])
...
def run_server():
    """Run the BodaBoda database server"""
    server_address = ('', 5000)
